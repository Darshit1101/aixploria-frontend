...
const GptGallery = () => {
    ...
    const filteredGpts = gpts.filter((gpt) => {
        ...
        const matchesSearch = gpt.title.toLowerCase().includes(search.toLowerCase());
        return matchesCategory && matchesSearch;
    });

    return (
        <>
            <div className='bg-black'><br /><br /><br />
                <div className="font-[Poppins] px-4 py-6 max-w-7xl mx-auto">
                    ...
                    <div className="flex text-white justify-between items-center mb-6">
                        ...
                        <span className="bg-gray-200 text-black text-sm px-3 py-1 rounded">{gpts.length} GPTs</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6 ">
                        <button
                            ...
                            className={`px-4 py-2 border-2 border-white text-white rounded-full ${selectedCategory === 'All' ? 'bg-orange-500 text-blue-600' : ''}`}
                        >
                            All
                        </button>
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`px-4 py-2 border-2 border-white text-white rounded-full ${selectedCategory === cat.name ? 'bg-orange-500 text-blue-600' : ''
                                    }`}
                            >
                                {cat.name}
                            </button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredGpts.map((gpt) => {
                            const link = gpt.link.startsWith("http") ? gpt.link : `https://${gpt.link}`;
                        ...
                        {filteredGpts.length === 0 && (
                            <p className="text-center col-span-full text-gray-500">No GPTs found.</p>
                        )}
                    </div>
